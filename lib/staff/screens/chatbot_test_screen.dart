...
class ChatbotTestScreen extends StatefulWidget {
  final String userId;
  const ChatbotTestScreen({super.key, required this.userId});
...
class _ChatbotTestScreenState extends State<ChatbotTestScreen> {
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  ...((results['results'] as List<Map<String, dynamic>>?) ?? [])
                      .map(
                        (result) => Padding(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.assessment, color: Color(0xFF001FF4)),
                const SizedBox(width: 8),
                const Text(
                ...
                Chip(
                  ...
                  backgroundColor:
                      double.parse(_testResults!['successRate']) >= 80
    ...
    return Expanded(
      child: Container(
        ...
        decoration: BoxDecoration(
          color: (color ?? const Color(0xFF001FF4)).withValues(alpha: 0.1),
          borderRadius: BorderRadius.circular(8),
        ),
        child: Column(
          children: [
            Icon(icon, color: color ?? const Color(0xFF001FF4)),
            ...
            Text(
              ...
              style: TextStyle(
                ...
                color: color ?? const Color(0xFF001FF4),
              ),
            ),
  ...
  List<Widget> _buildLanguageStats() {
    final langStats =
        _testResults!['languageStats'] as Map<String, Map<String, int>>;
    return langStats.entries.map((entry) {
      final lang = entry.key;
      final stats = entry.value;
      final total = (stats['passed'] ?? 0) + (stats['failed'] ?? 0);
      final rate = total > 0
          ? ((stats['passed'] ?? 0) / total * 100).toStringAsFixed(1)
          : '0.0';
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.description, color: Color(0xFF001FF4)),
                const SizedBox(width: 8),
                const Text(
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          gradient: const LinearGradient(
                            colors: [Color(0xFF001FF4), Color(0xFFEC4899)],
                          ),
  ...
  Widget _buildLanguageTestButton(String name, String code) {
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        ...
        side: const BorderSide(color: Color(0xFF001FF4)),
      ),
      child: Text(name),
    );
  }
