...
/// Seed 10 students (5 MCA + 5 MBA) to Firestore
/// Run this file once to populate the database
/// Command: dart run seed_students.dart
...
void main() async {
  ...
  print('📚 Adding ${allStudents.length} students to Firestore...\n');
  ...
  for (var student in allStudents) {
    try {
      ...
      if (existingStudent.docs.isNotEmpty) {
        print('⚠️  ${student['registrationNumber']} - ${student['firstName']} ${student['lastName']} already exists, skipping...');
        continue;
      }
      ...
      // Also add to users collection for login (using email as document ID)
      ...
      print('✅ ${student['registrationNumber']} - ${student['firstName']} ${student['lastName']} (${student['department']})');
      successCount++;
    } catch (e) {
      print('❌ Error adding ${student['registrationNumber']}: $e');
      errorCount++;
    }
  }
  ...
  print('=' * 50);
  print('✅ Successfully added: $successCount students');
  if (errorCount > 0) {
    print('❌ Errors: $errorCount');
  }
  print('\n📊 Summary:');
  print('   - MCA Students: ${mcaStudents.length}');
  print('   - MBA Students: ${mbaStudents.length}');
  print('   - Total: ${allStudents.length}');
  print('\n💡 You can now view these students in the admin dashboard!');
  print('=' * 50);
}
